...
def set_secret(service_client, arn, token):
    ...
    if current_dict["host"] != master_dict["host"] and not is_rds_replica_database(
        current_dict, master_dict
    ):
        # If current dict is a replica of the master dict, can proceed
        logger.error(
            "setSecret: Current database host %s is not the same host as/rds replica of master %s"
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(
                "setSecret: Successfully set password for %s in PostgreSQL DB for secret arn %s."
    ...
    if master_secret and (set(secret_dict.keys()) == set(["username", "password"])):
        ...
        if len(db_instance_info) != 0:
            secret_dict = get_connection_params_from_rds_api(
                secret_dict, db_instance_info
            )
            logger.info(
                "setSecret: Successfully fetched connection params for Master Secret %s from DescribeDBInstances API."
                % arn
            )
...
def is_rds_replica_database(replica_dict, master_dict):
    ...
    if master_dict["engine"] == "postgres":
        current_instance = get_instance_info_from_rds_api(
            replica_instance_id, rds_client
        )
    ...
    if master_dict["engine"] == "aurora-postgresql":
        ...
        is_reader_instance = any(
            replica_instance_id == replica_instance["instance_id"]
            and not replica_instance["is_writer"]
...
def get_cluster_info_from_master_host(master_dict, rds_client):
    ...
    if "cluster" in master_dict["host"].split(".")[1]:
        ...
        current_instance = get_instance_info_from_rds_api(
            master_instance_id, rds_client
        )
        if not current_instance or "DBClusterIdentifier" not in current_instance:
            return {}
        cluster_info = get_cluster_info_from_rds_api(
            current_instance.get("DBClusterIdentifier"), rds_client
        )
...
def get_instance_info_from_rds_api(instance_id, rds_client):
    ...
    try:
        ...
        logger.error(
            "setSecret: Encountered API error while fetching connection parameters from DescribeDBInstances RDS API: %s"
            % err
        )
        raise Exception(
            "Encountered API error while fetching connection parameters from DescribeDBInstances RDS API: %s"
            % err
        )
    ...
    if len(instances) == 0:
        logger.error(
            "setSecret: %s is not a valid DB Instance ARN. No Instances found when using DescribeDBInstances RDS API to get connection params."
            % instance_id
        )
        raise ValueError(
            "%s is not a valid DB Instance ARN. No Instances found when using DescribeDBInstances RDS API to get connection params."
            % instance_id
        )
...
def get_cluster_info_from_rds_api(cluster_id, rds_client):
    ...
    try:
        ...
        logger.error(
            "setSecret: Encountered API error while fetching connection parameters from DescribeDBClusters RDS API: %s"
            % err
        )
        raise Exception(
            "Encountered API error while fetching connection parameters from DescribeDBClusters RDS API: %s"
            % err
        )
    # Verify the instance was found
    ...
    if len(clusters) == 0:
        logger.error(
            "setSecret: %s is not a valid DB Cluster ARN. No Clusters found when using DescribeDBClusters RDS API to get connection params."
            % cluster_id
        )
        raise ValueError(
            "%s is not a valid DB Cluster ARN. No Clusters found when using DescribeDBClusters RDS API to get connection params."
            % cluster_id
        )
...
def fetch_instance_arn_from_system_tags(service_client, secret_arn):
    ...
    if "Tags" not in metadata:
        logger.warning(
            "setSecret: The secret %s is not a service-linked secret, so it does not have a tag aws:rds:primarydbinstancearn or a tag aws:rds:primarydbclusterarn"
            % secret_arn
        )
        return {}
    ...
    for tag in tags:
        if (
            tag["Key"].lower() == "aws:rds:primarydbinstancearn"
            or tag["Key"].lower() == "aws:rds:primarydbclusterarn"
        ):
    ...
    if len(db_instance_info) == 0:
        logger.warning(
            "setSecret: DB Instance ARN not present in Metadata System Tags for secret %s"
            % secret_arn
        )
    elif len(db_instance_info["ARN"]) > MAX_RDS_DB_INSTANCE_ARN_LENGTH:
        logger.error(
            "setSecret: %s is not a valid DB Instance ARN. It exceeds the maximum length of %d."
            % (db_instance_info["ARN"], MAX_RDS_DB_INSTANCE_ARN_LENGTH)
        )
        raise ValueError(
            "%s is not a valid DB Instance ARN. It exceeds the maximum length of %d."
            % (db_instance_info["ARN"], MAX_RDS_DB_INSTANCE_ARN_LENGTH)
        )
...
def get_connection_params_from_rds_api(master_dict, master_instance_info):
    ...
    if master_instance_info["ARN_SYSTEM_TAG"] == "aws:rds:primarydbinstancearn":
        ...
        primary_instance = get_instance_info_from_rds_api(
            master_instance_info["ARN"], rds_client
        )
        ...
        # put connection parameters in master secret dictionary
        primary_instance = get_cluster_info_from_rds_api(
            master_instance_info["ARN"], rds_client
        )
...
def get_input_map_value(input_dict, field_name):
    ...
    if field_name in input_dict:
        ...
        if re.match(r"^[ -~]+$", raw_value) is not None:
            ...
            raise ValueError(
                '"%s" contains invalid characters. Only printable ASCII characters are allowed.'
                % field_name
            )
